...
function searchPlaces() {
    ...
    const bounds = new kakao.maps.LatLngBounds(
        new kakao.maps.LatLng(33.08, 124.58), // 남서쪽 좌표 (제주도)
        new kakao.maps.LatLng(38.38, 131.87)  // 북동쪽 좌표 (독도)
    );
    ...
    places.keywordSearch(keyword, function (data, status) {
        if (status === kakao.maps.services.Status.OK) {
            ...
            // 1. 서버로 placeAPIid와 placeName을 먼저 저장 요청
            fetch('/rest/map/places/saveInfo', {
                method: 'POST',
                headers: {
                ...
                .then(() => {
                    ...
                    // 2. 서버로부터 이미지가 있는 Place ID 목록을 가져오기
                                ...
                                $.ajax({
                                    ...
                                    success: function (res) {
                                        ...
                                        if (!res || !res.src) {
                                            ...
                                            return;  // 이미지가 없으면 업데이트 요청을 하지 않음
                                        }
...
async function displayPlaceInfo(place) {
    const replyTypeNo = 3; 
    const reply = await getReplyForPlace(place.id, replyTypeNo);

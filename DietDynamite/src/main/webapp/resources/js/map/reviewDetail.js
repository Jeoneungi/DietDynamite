...
function showUpdateReply(no, el) {
  const replyUpdateModal = $('#updateModal');
  replyUpdateModal.find(".modal-body").html(`
        <textarea name="update-reply-content" rows="5" cols="30" placeholder="수정할 댓글을 입력해주세요" class="form-control"></textarea>
    `);

  replyUpdateModal.modal('show');

  replyUpdateModal.find(".acceptBtn").one("click", () => updateReply(no, replyUpdateModal));
}

function updateReply(no, replyUpdateModal) {
  const updateReplyContent = replyUpdateModal.find("[name='update-reply-content']").val();
  ...
  if (updateReplyContent.trim() !== "") {
    // Bootstrap Modal 인스턴스를 가져와서 모달을 닫음
    ...
    fetch(`/reply`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ replyNo: no, replyContent: updateReplyContent })
    })
      .then(response => response.text())
      .then(result => {
        if (result > 0) {
